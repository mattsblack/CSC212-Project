 ...
 * and logger for handling robot undocking operations.
 *
 ...
 *
 * This method handles the state machine for undocking the robot. It checks if
 * the robot is docked, waits for the action server to be ready,
 * sends undocking commands when appropriate, and monitors the result.
 *
 ...
 *         - FAILURE: Undocking attempt has failed
 */
State UndockBehavior::execute(const Data & data)
{
    // Make sure we are docked - no point in trying to undock if already undocked
    ...
    if (!m_undock_action_sent) {
        ...
        // Configure callbacks to handle the asynchronous action responses
...
{
    // This behavior is being cancelled, so send a cancel request to the undock action server if it's running
    // We only cancel if:

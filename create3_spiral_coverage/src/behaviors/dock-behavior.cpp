 ...
 * and logger for handling robot docking operations.
 *
 ...
 *
 * This method handles the state machine for docking the robot. It checks if
 * the robot is already docked, waits for the action server to be ready,
 * sends docking commands when appropriate, and monitors the result.
 *
 ...
 *         - FAILURE: Docking attempt has failed
 */
State DockBehavior::execute(const Data & data)
{
    // Make sure we are not docked - no point in trying to dock if already docked

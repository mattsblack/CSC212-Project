...
Create3SpiralCoverageNode::Create3SpiralCoverageNode()
: rclcpp::Node("create3_spiral_coverage")
{
    m_coverage_action_server = rclcpp_action::create_server<CoverageAction>(
        ...
        "coverage",
        std::bind(&Create3SpiralCoverageNode::handle_goal, this, _1, _2),
        std::bind(&Create3SpiralCoverageNode::handle_cancel, this, _1),
        std::bind(&Create3SpiralCoverageNode::handle_accepted, this, _1));
    ...
    m_dock_subscription = this->create_subscription<DockMsg>(
        ...
        std::bind(&Create3SpiralCoverageNode::dock_callback, this, _1));

    m_hazards_subscription = this->create_subscription<HazardMsg>(
        "hazard_detection",
        rclcpp::SensorDataQoS(),
        std::bind(&Create3SpiralCoverageNode::hazards_callback, this, _1));

    m_ir_opcode_subscription = this->create_subscription<OpCodeMsg>(
        "ir_opcode",
        rclcpp::SensorDataQoS(),
        std::bind(&Create3SpiralCoverageNode::ir_opcode_callback, this, _1));

    m_odom_subscription = this->create_subscription<OdometryMsg>(
        "odom",
        rclcpp::SensorDataQoS(),
        std::bind(&Create3SpiralCoverageNode::odom_callback, this, _1));

    m_kidnap_subscription = this->create_subscription<KidnapMsg>(
        "kidnap_status",
        rclcpp::SensorDataQoS(),
        std::bind(&Create3SpiralCoverageNode::kidnap_callback, this, _1));
...
Create3SpiralCoverageNode::handle_goal(
...
Create3SpiralCoverageNode::handle_cancel(
...
void Create3SpiralCoverageNode::handle_accepted(const std::shared_ptr<GoalHandleCoverage> goal_handle)
{
...
void Create3SpiralCoverageNode::execute(const std::shared_ptr<GoalHandleCoverage> goal_handle)
{
    ...
    SpiralCoverageStateMachine::CoverageOutput output;
...
bool Create3SpiralCoverageNode::reflexes_setup()
{
...
bool Create3SpiralCoverageNode::ready_to_start()
{
...
void Create3SpiralCoverageNode::dock_callback(DockMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
...
void Create3SpiralCoverageNode::hazards_callback(HazardMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    m_last_hazards = *msg;
}

void Create3SpiralCoverageNode::ir_opcode_callback(OpCodeMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    m_last_opcodes.push_back(*msg);
}

void Create3SpiralCoverageNode::kidnap_callback(KidnapMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    m_last_kidnap = *msg;
}

void Create3SpiralCoverageNode::odom_callback(OdometryMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    m_last_odom = *msg;
}
